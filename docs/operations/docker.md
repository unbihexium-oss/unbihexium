...
| Stage | Work |
| --- | --- |
| `builder` | Creates a virtual environment in `/opt/venv`; installs the dependencies from two hashed lock files (Section 2.2) with `--only-binary=:all: --require-hashes`; builds the wheel of the package and installs it without dependencies; runs `pip check` and imports `torch`, `onnxruntime` and `unbihexium.serving.app` so that a broken environment fails the build |
