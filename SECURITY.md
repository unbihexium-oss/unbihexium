...
- pip-audit checks [requirements.txt](requirements.txt) against the PyPI advisory database on pushes to main, pull requests and weekly. Its findings are informational: they are reported in the job log and do not fail the build.
...
- Bandit scans [src/](src/) with the settings in [pyproject.toml](pyproject.toml) on pushes to main, pull requests and weekly. Like pip-audit, it is informational and does not fail the build.
